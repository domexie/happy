...
interface ExtendedNavigationOptions extends Partial<StackHeaderProps['options']> {
...
// Component wrapper for navigation header (web version using @react-navigation/stack)
const NavigationHeaderComponent: React.FC<StackHeaderProps> = React.memo((props) => {
    ...
    // Extract static style from headerTitleStyle (ignore animated values)
    const titleStyle = options.headerTitleStyle && typeof options.headerTitleStyle === 'object' && !('_value' in options.headerTitleStyle)
    ...
    if (options.headerTitle) {
        if (typeof options.headerTitle === 'string') {
            title = (
                <Text style={[
                    { fontSize: 17, fontWeight: '600', textAlign: 'left', color: options.headerTintColor || '#000' },
    ...
    } else if (back && !shouldHideBackButton) {
...
// Export a render function for React Navigation (web version)
export const createHeader = (props: StackHeaderProps) => {
